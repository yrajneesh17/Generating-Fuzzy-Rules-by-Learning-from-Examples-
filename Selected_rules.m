...
%Calculating the Degree for given datasets Input x
...
%Expert rules are not given so we are generating rules from data pairs only .
%For conflicting rule in rule base ,assign the maximum degree for the conflict group.
...
final_rules= FINAL_RULES(12:18,:) ;                                                                        % and sort the Data
...
ylabel('y (in meters)');
ylim([0 65]);
title('Truck Trajectories using the Selected linguistic rules');
